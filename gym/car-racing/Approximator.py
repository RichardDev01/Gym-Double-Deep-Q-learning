...
class Approximator:
    ...
    def __init__(self):
        ...
        self.model_path = os.path.dirname(os.getcwd()) + '/car-racing/models/'
    ...
    def create_network_q1(self, input_size: int = 8, output_size: int = 4, middle_layer_size: int = 12):
        ...
        self.q_network_1 = nn.Sequential(
            nn.Conv2d(1, 96, 96, kernel_size=(7, 7), stride=3),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(1, 48, 48, kernel_size=(4, 4)),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Flatten(),
            nn.Linear(out_features=len(self.action_space))
        ).to(self.device)
    ...
    def create_network_q2(self, input_size: int = 8, output_size: int = 4, middle_layer_size: int = 12):
        ...
        self.q_network_1 = nn.Sequential(
            nn.Conv2d(96, 96, kernel_size=(7, 7), stride=3),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(48, 48, kernel_size=(4, 4)),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Flatten(),
            nn.Linear(out_features=216),
            nn.ReLU(),
            nn.Linear(len(self.action_space))
        ).to(self.device)
